...
<div class="content-info-nhan-su">
	<div class="content-table">
		<div class="table-name">
			<h3>Quản lý thông tin nhân sự trực thuộc phòng ban</h3>
		</div>
		<div>
		...
		@endphp
		<div class="table-btn">
			<a class="btn bg-btn"  href="/insert-nhan-su">Thêm nhân sự</a>
		</div>
		@php
			}
		@endphp
			...
			@foreach ($phongban as $item)
				...
				<tr>
					...
					<td>
						...
						<a href="/delete-nhan-su/{{ $item->manv }}" onclick="confirmDelete()" class="btn btn-danger delete-product" title="xóa nhân viên {{$item->hoten}}">
						...
						<i>Quá trình công tác</i>
							<thead class="bg-gradient bg-secondary ">
								...
								@php
									if(Session::get('user')['role_id'] == 1){
								@endphp
								<a href="/chuyen-cong-tac/{{ $item->manv }}" class="btn btn-warning" title="Chuyển Công Tác Nhân Viên[ {{$item->hoten}} ]">
									Chuyển Công Tác 
								</a> 
								@php
									}
