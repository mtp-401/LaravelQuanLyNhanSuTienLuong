...
<div style="width: 98%;">
	<div class="content-info-nhan-su">
		...
		<div class="card">
			<h2 class="card-header text-primary">BÁO CÁO LƯƠNG </h2>
			<div class="card-body">
		...
		<div class="form-row">
			<div class="table-btn col-md-12">
				<a class="btn btn-success ml-5" href="/Xuat-bao-cao-thong-ke-luong">Xuất Tất Cả</a>
			</div>
		</div>
		<div class="form-row m-5">
			<div class="table-btn col-md-4">
				<select name="khoa" id="khoa">
					...
					@endphp
						<option value="{{$value->maphongban}}">{{$value->tenphongban}}</option>
					@php 
						}
					@endphp
				</select>
			...
			<div class="table-btn col-md-4">
				<button class="btn btn-danger" onclick="searchThangNam()">Xuất</button>
			</div>
		</div>
	</div>
...
function searchThangNam(){
	...
	if(thang <=12 && thang >=1 && nam>=1990 && nam<=9999){
		...
		swal("Dowload Thành Công!", "Bảng Lương Tháng: "+thang+" Năm: "+nam, "success");
		var input, filter, table, tr, td, i, txtValue;
		input = nam+"-"+thang;
		if(thang <10){
			input = nam+"-0"+thang;
		}
		window.location.href = '/Xuat-bao-cao-thong-ke-luong-filter/'+khoa+'_'+thang+'_'+nam;
	}else{
